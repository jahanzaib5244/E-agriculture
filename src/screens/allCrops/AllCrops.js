...
export default function AllCrops({navigation}) {
  ...
  const deleteCrop = async item => {
    try {
      setdeleting(true);
      const res = await firestore().collection('crops').doc(item?.id).delete();
      console.log(res);
      setdeleting(false);
    } catch (error) {
      console.log(error);
  ...
  return (
    <View style={styles.root}>
      <TouchableOpacity
        onPress={() => navigation.navigate(NavStrings.AddCrop)}
        activeOpacity={0.8}
              ...
              <View
                ...
                <View style={styles.detailContainer}>
                  ...
                  <View style={styles.cradBtnContainer}>
                    <TouchableOpacity
                      onPress={() =>
                        navigation.navigate(NavStrings.AddCrop, {item})
                      }
