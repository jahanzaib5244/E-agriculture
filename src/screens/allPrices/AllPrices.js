...
export default function AllPrices({navigation}) {
  const price = useSelector(state => state.AuthReducer.AllPrices);
  const [deleting, setdeleting] = useState(false);

  const deleteCrop = async item => {
    try {
      setdeleting(true);
      await firestore().collection('price').doc(item?.id).delete();
      setdeleting(false);
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <View style={styles.root}>
      <TouchableOpacity
        onPress={() => navigation.navigate(NavStrings.AddPrice)}
        activeOpacity={0.8}
        style={styles.addBtnContainer}>
        <Image source={ImgPath.Add} style={styles.addIcon} />
        <Text style={styles.addTxt}>Add new Price</Text>
      </TouchableOpacity>
      <FlatList
        data={price}
        ListEmptyComponent={() => (
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyTxt}>No Price Available</Text>
          </View>
        )}
              ...
              <View
                ...
                <View style={styles.detailContainer}>
                  <View>
                    ...
                    <Text style={styles.productDes}>{item?.location}</Text>
                    <Text style={styles.productDes}>{item?.price}</Text>
                    <Text style={styles.productDes}>
                      {moment(item?.timestamp).format('dddd D-M-yyyy')}
                    </Text>
                  </View>
                  <View style={styles.cradBtnContainer}>
                    <TouchableOpacity
                      onPress={() =>
                        navigation.navigate(NavStrings.AddPrice, {item})
                      }
